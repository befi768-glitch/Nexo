...
function profileEmbed(member, user, settings = {}) {
  const tx = k => t(settings, k);
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: `${emoji.coin()} ${tx("coin")}`, value: `${user.coins || 0}`, inline: true },
      ...
      { name: `${emoji.twinkle()} Cosmetic`, value: `Frame: ${frame}\nTitle: ${title || (settings.language === "en" ? "Not equipped" : "Chưa trang bị")}`, inline: true },
      { name: `${emoji.badge()} Bộ sưu tập`, value: badges }
    )
    .setThumbnail(member.displayAvatarURL({ size: 256 }))
    .setColor(0x63c5da)
    .setFooter({ text: `Nexo v2.7 • ${tx("footer")}` });
}
function helpEmbed(settings = {}) { const tx = k => t(settings, k); return new EmbedBuilder().setTitle(`${emoji.companion()} ${tx("helpTitle")}`).setDescription(tx("helpDesc")).addFields({ name: tx("start"), value: `/profile — ${tx("profile")}\n/daily — ${tx("daily")}\n/quest list — ${tx("quests")}` }, { name: tx("progress"), value: `/leaderboard — ${tx("leaderboard")}\n/badges — ${tx("badges")}\n/companion — ${tx("companion")}\n/progress — ${tx("serverProgress")}\n/shop — ${tx("shop")}\n/inventory — ${tx("inventory")}` }, { name: tx("admin"), value: `/settings view — ${tx("settings")}\n/settings set — ${tx("settings")}\n/data export — ${tx("dataExport")}` }, { name: tx("emoji"), value: tx("emojiDesc") }).setColor(0x63c5da).setFooter({ text: `Nexo v2.7 • ${tx("footer")}` }); }
function settingsEmbed(guild) { const s=guild.settings; const tx=k=>t(s,k); return new EmbedBuilder().setTitle(`⚙️ ${tx("settingsTitle")}`).addFields({name:"XP",value:`${s.xpEnabled ? tx("enabled") : tx("disabled")} • Cooldown: ${s.cooldownMs ?? tx("default")} ms • Range: ${s.xpMin ?? config.xp.messageMin}-${s.xpMax ?? config.xp.messageMax}`,inline:false},{name:"Daily",value:s.dailyEnabled?tx("enabled"):tx("disabled"),inline:true},{name:"Level-up channel",value:s.levelUpChannelId?`<#${s.levelUpChannelId}>`:tx("unset"),inline:true},{name:"Embed color",value:s.embedColor,inline:true},{name:tx("language"),value:s.language,inline:true},{name:tx("autoEmoji"),value:s.autoEmoji?tx("enabled"):tx("disabled"),inline:true}).setColor(parseInt(s.embedColor.replace("#",""),16)); }
...
async function handle(interaction) {
  ...
  if(interaction.commandName==="help") return interaction.reply({embeds:[helpEmbed(guild.settings)]});
  ...
  if(interaction.commandName==="shop"){const lines=config.economy.shop.map(x=>`**${x.id}** — ${x.name} • ${emoji.coin()} ${x.price}\n└ ${x.description}`);return interaction.reply({embeds:[new EmbedBuilder().setTitle(`${emoji.shop()} Nexo Cosmetic Shop`).setDescription(lines.join("\n\n")).setFooter({text:"Dùng /buy item:<ID> để mua."}).setColor(0xf2c94c)]});}
  if(interaction.commandName==="buy"){const id=interaction.options.getString("item",true);const item=config.economy.shop.find(x=>x.id===id);if(!item)return interaction.reply({content:"Không tìm thấy vật phẩm.",ephemeral:true});if(user.inventory.includes(id))return interaction.reply({content:"Bạn đã sở hữu vật phẩm này.",ephemeral:true});if(!(await db.spendCoins(guildId,interaction.user.id,item.price)))return interaction.reply({content:`Không đủ Coin. Cần ${emoji.coin()} ${item.price}.`,ephemeral:true});user.inventory.push(id);await db.saveUser(user);return interaction.reply({content:`${emoji.shop()} Đã mua **${item.name}** với ${emoji.coin()} ${item.price}.`});}
  if(interaction.commandName==="inventory"){const items=user.inventory.map(id=>config.economy.shop.find(x=>x.id===id)).filter(Boolean);return interaction.reply({embeds:[new EmbedBuilder().setTitle(`${emoji.inventory()} Kho cosmetic`).setDescription(items.length?items.map(x=>`• **${x.name}** — \`${x.id}\``).join("\n"):"Bạn chưa có cosmetic nào.").addFields({name:`${emoji.coin()} Coin`,value:String(user.coins||0)}).setColor(0x8c7ae6)]});}
  if(interaction.commandName==="equip"){const id=interaction.options.getString("item",true);const item=config.economy.shop.find(x=>x.id===id);if(!item||!user.inventory.includes(id))return interaction.reply({content:"Bạn chưa sở hữu vật phẩm này.",ephemeral:true});user.equipped[item.type]=id;await db.saveUser(user);return interaction.reply({content:`${emoji.twinkle()} Đã trang bị **${item.name}**.`});}
  if(interaction.commandName==="gift"){const target=interaction.options.getUser("user",true), amount=interaction.options.getInteger("amount",true);if(target.bot||target.id===interaction.user.id)return interaction.reply({content:"Chỉ có thể tặng Coin cho thành viên khác.",ephemeral:true});if(!(await db.spendCoins(guildId,interaction.user.id,amount)))return interaction.reply({content:"Bạn không đủ Coin.",ephemeral:true});await db.addCoins(guildId,target.id,amount);return interaction.reply({content:`${emoji.gift()} ${interaction.user} đã tặng **${emoji.coin()} ${amount}** cho ${target}.`});}
  ...
  if(interaction.commandName==="data") {
    ...
    if(sub==="export") {
      ...
      return interaction.reply({content:`${emoji.memory()} Dữ liệu server đã được xuất.`,files:[new AttachmentBuilder(buffer,{name:`nexo-${guildId}-export.json`})],ephemeral:true});
    }
    if(sub==="backup") {
      const file=await db.backup(`guild-${guildId}`);
      return interaction.reply({content:`${emoji.reward()} ${t(guild.settings,"backupCreated")}${file.remote ? ` • Remote: ${file.remote}` : ` • Local: ${file.local}`}`,ephemeral:true});
    }
    if(sub==="reset-user") {
      const target=interaction.options.getUser("user",true); await db.resetUser(guildId,target.id);
      return interaction.reply({content:`${emoji.check()} Đã reset dữ liệu Nexo của ${target}.`,ephemeral:true});
    }
    if(sub==="reset-server") {
      ...
      return interaction.reply({content:`${emoji.locked()} Đã reset toàn bộ dữ liệu Nexo của server.`,ephemeral:true});
    }
  }
